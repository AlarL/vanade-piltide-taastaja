...
export const ResponsibilityBanner: React.FC<ResponsibilityBannerProps> = ({ quota }) => {
  return (
    <div
      ...
      className="w-full mb-6 rounded-2xl bg-amber-50/80 border border-amber-200/90 p-3.5 sm:px-5 text-xs text-amber-950 shadow-2xs"
    >
        ...
        <div className="flex items-center gap-2.5 text-center md:text-left">
          ...
          <div>
            <span className="font-semibold tracking-tight">
              Kasuta ainult siis, kui sul on seda tõesti vaja.
            </span>
            <span className="block sm:inline sm:ml-2 text-amber-900/80 font-normal">
              See kulutab energiat ja ressursse. Ole vastutustundlik.
            </span>
          </div>
        </div>

        {/* Right side: In-memory processing & daily quota */}
        <div className="flex flex-wrap items-center justify-center md:justify-end gap-2 text-[11px] text-amber-900/85">
          ...
          {quota && (
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-white/80 border border-amber-200/80 text-stone-800 font-mono text-[10px]">
